...
describe('製品', () => {
  describe("は名前を持つ", () => {
    test('名前"製品名XYZ"の製品は、Product#nameで"製品名XYZ"を返す', () => {
      const product = new Product({name: "製品名XYZ", category: "", price: 0})
  ...
  describe("はカテゴリを持つ", () => {
    test('カテゴリ"ワードプロセッサ"の製品は、Product#categoryで"ワードプロセッサ"を返す', () => {
      const product = new Product({category: "ワードプロセッサ", name: "", price: 0})
  ...
  describe("はカテゴリを持つ", () => {
    test('価格"10,000円"の製品は、Product#priceで"10000"を返す', () => {
      const product = new Product({price: 10000, name: "", category: ""})
